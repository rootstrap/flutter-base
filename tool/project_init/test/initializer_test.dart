...
void main() {
  ...
  test('lockedPackageNames reads only the packages section', () {
    const lock = """
# Generated by pub
packages:
  bloc:
...
sdks:
  ...
  flutter: ">=3.47.0"
""";
    expect(lockedPackageNames(lock), {'bloc', 'sky_engine'});
  });
  ...
  group(
    ...
    () {
      ...
      test('rejects indirect dependencies and writes nothing', () {
        // bloc is only reached through flutter_bloc; taking its name makes
        // pub fail to resolve the workspace after init.
        final before = _snapshot(copy);
        const taken = ProjectIdentity(
          displayName: 'Ok',
          packageName: 'bloc',
          androidApplicationId: 'com.example.ok',
          androidNamespace: 'com.example.ok',
          iosBundleId: 'com.example.ok',
        );

        expect(
          () => ProjectInitializer(copy, formatGeneratedCode: false).run(taken),
          throwsA(
            isA<InitException>().having(
              (e) => e.message,
              'message',
              contains('"bloc" is already used'),
            ),
          ),
        );
        expect(_snapshot(copy), before);
      ...
      test(
        'resolvedPackageNames lists every package in pubspec.lock',
        skip: File('${repositoryRoot.path}/pubspec.lock').existsSync()
            ? false
            : 'no pubspec.lock; run melos bootstrap',
        () {
          final locked = lockedPackageNames(
            File('${repositoryRoot.path}/pubspec.lock').readAsStringSync(),
          );
          expect(
            locked.difference(resolvedPackageNames),
            isEmpty,
            reason:
                'Add these names to tool/project_init/lib/src/'
                'resolved_packages.dart.',
          );
        },
      );
